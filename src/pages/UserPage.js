...
function UserPage() {
  ...
  const { paginationOptions, whispererWhispers } = useSelector(
  ...
  useEffect(() => {
    dispatch(
      getWhispererWhispers({ page: 1, limit: 10, userId: params.userId })
    );
  ...
  useEffect(() => {
    ...
    socket.on('private-message', (data) => {
      ...
      localStorage.setItem(
        'chatMessages',
        JSON.stringify([...chatMessages, data])
      );
    });
  ...
  const fetchMoreData = () => {
    dispatch(getWhispererWhispers({ nextPage, limit, userId: params.userId }));
  };
  ...
  function handleFollowUnfollow() {
    if (followStatus) {
      dispatch(
        unfollowUser({ userId: params.userId, whispererId: auth.user._id })
      );
      setChange(!change);
    } else {
      dispatch(
        followUser({ userId: params.userId, whispererId: auth.user._id })
      );
      setChange(!change);
    }
  }
  ...
  const handleSendMessage = () => {
    ...
    const messageInfo = {
      ...
      image_url: auth.user.image_url
        ? auth.user.image_url
        : 'https://plus.unsplash.com/premium_photo-1682140993556-f263e434000b?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OXx8Y29kaW5nfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60',
      date,
  ...
  return (
    <main className="profile">
      <div className="profile-header">
        ...
        <div>
          <span>{`Followers: ${user ? size(user.followers) : ''}`}</span>
          <span>{`Following: ${user ? size(user.following) : ''}`}</span>
        </div>
      </div>
