  ...
  return (
    <main className='single-whisper'>
      ...
      <div>
        ...
        <h2>{whisper.title}</h2>
        {/* <div><span>{`${whisper.whisperer.firstname} ${whisper.whisperer.lastname}`}</span><span>{whisper.createdAt ? formatDate(whisper.createdAt): 'October 2023'}</span></div> */}
        <Link className='whisper-whisperer-link' to={`/whisperers/${whisper.whispererId}`} >{whisper.whisperer.username}</Link>
        <p>{whisper.whisper}</p>
